#!/bin/bash
# Lanza el servidor dedicado y el cliente en secuencia (Linux)
# Uso: doble click o ./lanzar_todo.sh
...
pkill -f "godot.*--path.*$SCRIPT_DIR" 2>/dev/null
sleep 1

echo "Iniciando servidor dedicado..."
"$GODOT_BIN" --path "$SCRIPT_DIR" --headless --server > "$SERVER_LOG" 2>&1 &
SERVER_PID=$!
...
"$GODOT_BIN" --path "$SCRIPT_DIR" > "$CLIENT_LOG" 2>&1 &
CLIENT_PID=$!
