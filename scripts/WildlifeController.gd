...
func setup(kind: String, points: Array) -> void:
	...
	if animal_type == "wolf":
		...
		_wolf_hunger = randf_range(40.0, 80.0)
		_wolf_hunger_threshold = randf_range(50.0, 80.0)
...
func _process(delta: float) -> void:
	...
	if animal_type == "wolf":
		_update_wolf_sounds(delta)
		_wolf_hunger = max(0.0, _wolf_hunger - delta * 0.15)
		if _wolf_hunger <= 0.0:
		...
		if _wolf_eating_timer > 0.0:
			...
			_wolf_hunger = min(60.0, _wolf_hunger + delta * 5.0)
			if _wolf_hunger >= 60.0 or _wolf_eating_timer <= 0.0:
	...
	if _player != null and is_instance_valid(_player) and _chase_cooldown <= 0.0 and not _player.get_meta("proxy_dead", false) and not _player.get_meta("in_built_shelter", false):
		...
		if dist_to_player < 45.0:
			...
			if sep.length() > 0.01:
				target += sep
			return {"target": target, "speed": speed}
	# Priority 1: eat nearby corpse when hungry (lower priority than chasing player)
	if is_hungry:
...
func interact(player: Node) -> void:
	...
	if not has_knife:
		...
		return
	# Puppet: notify server to gut and sync drops to all clients (server spawns meat)
	if is_puppet:
		_gutted = true
		_rot_timer = 5.1
		...
		# Server will spawn meat and sync via animal_gutted RPC — just remove puppet after animation
